...
/**
 * The gate. Ported from backend/src/gate/gate.ts. Replicates the
 * hauska-mcp-server access-policy pattern: a caller presents a product key via
 ...
 *
 * The hard guardrail (honesty guardrail / two-flywheel partition): a missing or
 * unrecognized key resolves to the PUBLIC-ONLY subject, so tenant-private atoms
 * (the operating pro forma, the deal molecule) NEVER leak into a public context.
 */
...
/**
 * Demo product-key table. The registry is a static, in-memory map keyed by the
 * raw product key so the demo runs with no datastore. Keys are read from the
 * request header only and are never logged.
 *
