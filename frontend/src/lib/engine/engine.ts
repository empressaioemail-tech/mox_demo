import type { AtomStore } from "./atoms";
import type { CalibrationEvent, CalibrationSummary } from "./calibration";
import { summaryFor } from "./calibration";
import type { BindContext, PopulatedComponent } from "./catalog";
import { CATALOG_BY_KIND } from "./catalog";
import type { AccessSubject } from "./gate";
import { filterAtoms } from "./gate";
import type {
  AssemblyProvider,
  AtomInventoryItem,
  ComponentSelection,
...
/**
 * The adaptive-assembly engine. Ported from backend/src/engine/engine.ts.
 * Pipeline:
 ...
 *      bound atoms — computed from the CLIENT-ACCUMULATED events passed in
 *      (stateless serverless), so the earning loop is visible after a correction
 *      without a persistent server. Honest: signal count, not earned-calibrated.
 */
...
export class AdaptiveEngine {
  ...
  /**
   * @param events client-accumulated calibration events (stateless serverless);
   *   reflected back into each component's bound atoms. Empty for a cold view.
   */
  async assemble(
    intent: string,
