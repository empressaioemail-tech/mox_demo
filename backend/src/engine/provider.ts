import type { ComponentKind } from "./catalog.js";
...
export class AnthropicProvider implements AssemblyProvider {
  ...
  async plan(intent: string, inventory: AtomInventoryItem[]): Promise<AssemblyPlan> {
    // Dynamic import so the SDK is only loaded when a key is present; the demo
    // typechecks and runs in mock mode without the dependency installed.
